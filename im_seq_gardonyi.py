...
def main():
    ...
    # set map "gardonyiter"
    map_string = "gardonyiter"

    # get current time and make new dir
    current_time = datetime.now().strftime("%m_%d_%H_%M_%S")
    os.makedirs(f"_out/gardonyiter/sequences/{current_time}")
    print(f"_out/gardonyiter/sequences/{current_time}")
    ...
    try:
        ...
        for spawn_position in spawn_indices:
            ...
            for i in range(num_runs):
                ...
                vehicle = world.spawn_actor(bp, carla.Transform(location=carla.Location(x=-65.61803436279297,
                                                                                        y=39.20486831665039,
                                                                                        z=1,
                                                                                        ),
                                                                rotation=carla.Rotation(yaw=-50,
                                                                                        roll=0,
                                                                                        pitch=0)))
                ...
                export_basepath = f"_out/gardonyiter/sequences/{current_time}/{j}_{i+1}"
                ...
                with CarlaSyncMode(world, *sensor_list, fps=30) as synchronizer:
                    ...
                    while True:
                        ...
                        cv.imwrite(f'{export_basepath}/semseg/{tick}.png', semseg_mask)
